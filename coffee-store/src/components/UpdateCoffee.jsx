...
const UpdateCoffee = () => {
  ...
  const { _id, name, quantiy, price, photo, taste, supplier, details } =
    userData;
  ...
  const handleUpdateCoffe = (e) => {
    ...
    fetch(`${import.meta.env.VITE_NODE_SERVER_URL}/coffees/${_id}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (data.modifiedCount > 0) console.log("after modified", data);
      });
  };
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
          ...
          <fieldset className="fieldset bg-base-200 border-base-300 rounded-box border p-4">
            ...
            <input
              ...
              name="quantiy"
              className="input w-full"
              defaultValue={quantiy}
              placeholder="Enter quantiy name"
            />
          </fieldset>
          <fieldset className="fieldset bg-base-200 border-base-300 rounded-box border p-4">
